...
class FileBasedServiceRegistry {
  ...
  register(serviceName, serviceInfo) {
    ...
    console.log(
      `Serviço registrado: ${serviceName} - ${serviceInfo.url} (PID: ${process.pid})`
    );
  ...
  async performHealthChecks() {
    ...
    console.log(
      `Executando health checks de ${Object.keys(services).length} serviços...`
    );
    ...
    for (const [serviceName, service] of Object.entries(services)) {
      try {
        ...
        console.error(
          `Health check falhou para ${serviceName}:`,
          error.message
        );
