    ...
    try {
      ...
      if (response.data.success) {
        ...
        products.forEach((product, index) => {
          ...
          console.log(
            `  ${index + 1}. ${product.name} - R$ ${product.price} (Estoque: ${
              product.stock
            })${tags}`
          );
        });
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (dashboard.services_status) {
          Object.entries(dashboard.services_status).forEach(
            ([serviceName, serviceInfo]) => {
              const status = serviceInfo.healthy ? "SAUDÁVEL" : "INDISPONÍVEL";
              console.log(
                `     ${serviceName}: ${status} (${serviceInfo.url})`
              );
            }
          );
        }
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (results.products?.available) {
          console.log(
            `   Produtos encontrados: ${results.products.results.length}`
          );
        ...
        if (results.users?.available) {
          console.log(
            `   Usuários encontrados: ${results.users.results.length}`
          );
      ...
      if (gatewayHealth.status === "fulfilled") {
        ...
        if (registryInfo.status === "fulfilled") {
          ...
          Object.entries(services).forEach(([name, info]) => {
            ...
            console.log(
              `   ${name}: ${status} (${info.url}) - uptime: ${uptime}s`
            );
          });
        } else {
          console.log(
            "   Erro ao buscar registry:",
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.log(
            "Login com admin falhou, continuando sem autenticação..."
          );
      ...
      if (authSuccessful && this.authToken) {
        ...
        try {
          ...
          if (newProduct.success) {
            await this.delay(1000);
            console.log(
              `Produto criado: ${newProduct.data.name} (ID: ${newProduct.data.id})`
            );
          }
        } catch (error) {
...
async function main() {
  ...
  try {
    if (args.includes("--health")) {
      ...
      const searchTerm = searchArg.includes("=")
        ? searchArg.split("=")[1]
        : "smartphone";
