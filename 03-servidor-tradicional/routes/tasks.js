...
router.get("/", async (req, res) => {
  try {
    ...
    const tasks = rows.map(
      (row) => new Task({ ...row, completed: row.completed === 1 })
    );
...
router.get("/stats/summary", async (req, res) => {
  try {
    const stats = await database.get(
      "SELECT COUNT(*) as total, SUM(CASE WHEN completed = 1 THEN 1 ELSE 0 END) as completed, SUM(CASE WHEN completed = 0 THEN 1 ELSE 0 END) as pending FROM tasks WHERE userId = ?",
      [req.user.id]
    );
