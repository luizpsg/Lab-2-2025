...
class TaskService {
  ...
  async getTasks(call, callback) {
    try {
      ...
      const result = await database.getAllWithPagination(
        sql,
        params,
        pageNum,
        limitNum
  ...
  async updateTask(call, callback) {
    try {
      const { token, task_id, title, description, completed, priority } =
        call.request;
      const user = await this.validateToken(token);
      ...
      const updateData = {
        ...
        completed:
          completed !== undefined ? completed : existingTask.completed === 1,
        priority:
          priority !== undefined
            ? ProtoLoader.convertFromPriority(priority)
  ...
  async deleteTask(call, callback) {
    try {
      ...
      const task = new Task({
        ...existingTask,
        completed: existingTask.completed === 1,
      });
