  ...
  describe("Autenticação", () => {
    ...
    test("deve fazer login com sucesso", async () => {
      ...
      const response = await client.login({
        identifier: client.currentToken
          ? "existing_user@grpc.com"
          : "test@grpc.com",
        password: "password123",
      });
    ...
    test("deve validar token corretamente", async () => {
      ...
      const validateTokenPromise = client.promisify(
        client.authClient,
        "ValidateToken"
      );
      const response = await validateTokenPromise({
  ...
  describe("Streaming", () => {
    beforeAll(async () => {
      ...
      try {
        ...
        console.log(
          "Falha ao criar tarefa para teste de stream:",
          error.message
        );
      }
    });
    ...
    test("deve receber stream de notificações", (done) => {
      ...
      stream.on("error", (error) => {
        if (!streamEnded) {
          ...
          clearTimeout(timeout);
          console.log(
            "Notification stream error (pode ser esperado):",
            error.message
          );
          finishTest();
        }
