...
class PerformanceBenchmark {
  ...
  async benchmarkGrpc(iterations = 100) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        ...
        if (i % 20 === 0 && i > 0) {
          console.log(
            `gRPC: ${i}/${iterations} completed (${successCount} success)`
          );
        }
      } catch (error) {
    ...
    console.log(
      `✅ Benchmark gRPC concluído: ${successCount}/${iterations} sucessos`
    );
  }
  ...
  async benchmarkRest(iterations = 100) {
    ...
    try {
      ...
      console.log(
        "⚠️ Servidor REST não disponível ou erro na configuração:",
        error.message
      );
      console.log(
        "   Para executar comparação completa, inicie o servidor do Roteiro 1 na porta 3000"
      );
      return;
    }
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        const response = await axios.get(`${baseUrl}/tasks?page=1&limit=10`, {
          headers,
        });
        const end = process.hrtime.bigint();
        ...
        if (i % 20 === 0 && i > 0) {
          console.log(
            `REST: ${i}/${iterations} completed (${successCount} success)`
          );
        }
      } catch (error) {
    ...
    console.log(
      `✅ Benchmark REST concluído: ${successCount}/${iterations} sucessos`
    );
  }
  ...
  calculateStats(times) {
    ...
    return {
      ...
      stdDev: Math.sqrt(
        times.reduce(
          (acc, time) => acc + Math.pow(time - sum / times.length, 2),
          0
        ) / times.length
      ),
    };
  ...
  printResults() {
    ...
    if (grpcStats && this.results.grpc.times.length > 0) {
      ...
      console.log(
        `   ├─ Requisições válidas: ${this.results.grpc.times.length}`
      );
      console.log(`   ├─ Erros: ${this.results.grpc.errors}`);
      console.log(
        `   ├─ Taxa de sucesso: ${(
          (this.results.grpc.times.length /
            (this.results.grpc.times.length + this.results.grpc.errors)) *
          100
        ).toFixed(1)}%`
      );
      ...
      console.log(
        `   ├─ Min/Max: ${grpcStats.min.toFixed(2)}ms / ${grpcStats.max.toFixed(
          2
        )}ms`
      );
      ...
      console.log(
        `   └─ Total bytes: ${this.results.grpc.totalBytes.toLocaleString()}`
      );
    } else {
      console.log("\n🔧 gRPC/Protocol Buffers:");
    ...
    if (restStats && this.results.rest.times.length > 0) {
      console.log("\n🌐 REST/JSON:");
      console.log(
        `   ├─ Requisições válidas: ${this.results.rest.times.length}`
      );
      console.log(`   ├─ Erros: ${this.results.rest.errors}`);
      console.log(
        `   ├─ Taxa de sucesso: ${(
          (this.results.rest.times.length /
            (this.results.rest.times.length + this.results.rest.errors)) *
          100
        ).toFixed(1)}%`
      );
      ...
      console.log(
        `   ├─ Min/Max: ${restStats.min.toFixed(2)}ms / ${restStats.max.toFixed(
          2
        )}ms`
      );
      ...
      console.log(
        `   └─ Total bytes: ${this.results.rest.totalBytes.toLocaleString()}`
      );
    } else {
      console.log("\n🌐 REST/JSON:");
    ...
    if (
      grpcStats &&
      restStats &&
      this.results.grpc.times.length > 0 &&
      this.results.rest.times.length > 0
    ) {
      const latencyImprovement =
        ((restStats.mean - grpcStats.mean) / restStats.mean) * 100;
      const bandwidthSavings =
        ((this.results.rest.totalBytes - this.results.grpc.totalBytes) /
          this.results.rest.totalBytes) *
        100;

      console.log("\n🏆 ANÁLISE COMPARATIVA:");
      console.log(
        `   ├─ Latência: gRPC é ${Math.abs(latencyImprovement).toFixed(1)}% ${
          latencyImprovement > 0 ? "mais rápido" : "mais lento"
        } que REST`
      );
      console.log(
        `   ├─ Diferença média: ${Math.abs(
          restStats.mean - grpcStats.mean
        ).toFixed(2)}ms`
      );
      console.log(
        `   ├─ Bandwidth: gRPC usa ${Math.abs(bandwidthSavings).toFixed(1)}% ${
          bandwidthSavings > 0 ? "menos" : "mais"
        } dados`
      );
      console.log(
        `   ├─ Throughput gRPC: ${(1000 / grpcStats.mean).toFixed(1)} req/s`
      );
      console.log(
        `   ├─ Throughput REST: ${(1000 / restStats.mean).toFixed(1)} req/s`
      );
      ...
      console.log("\n🏆 ANÁLISE COMPARATIVA:");
      console.log(
        "   └─ ⚠️ Comparação não disponível - dados insuficientes de um ou ambos protocolos"
      );
    }
...
async function runBenchmark() {
  ...
  console.log(
    `🚀 Iniciando benchmark com ${iterations} iterações por protocolo`
  );
  ...
  try {
    ...
    console.log(
      "   Para comparação completa, execute o servidor do Roteiro 1 na porta 3000"
    );
  }
